...
- **最小依赖**：核心依赖 5 个 pip 包（funasr_onnx、modelscope、onnxruntime、click、tqdm）
...
| 语音识别 | funasr_onnx (Paraformer ONNX) | 中文 SOTA 准确率，无需 PyTorch |
...
首次使用时从 ModelScope 自动下载 Paraformer 模型（约 1.2GB），后续无需再次下载。终端显示进度条。零配置——不需要手动下载、不需要设置环境变量。
...
三模型均从 ModelScope 自动下载：

| 模型 | ID | 功能 |
|------|-----|------|
| Paraformer-ONNX | `iic/speech_paraformer-large_asr_nat-zh-cn-16k-common-vocab8404-onnx` | 语音转文字 |
| FSMN-VAD-ONNX | `iic/speech_fsmn_vad_zh-cn-16k-common-onnx` | 静音切分 |
| CT-Transformer-ONNX | `iic/punc_ct-transformer_zh-cn-common-vocab272727-onnx` | 标点恢复 |

推理流程：

1. `Fsmn_vad(audio_path)` → 检测语音段
2. `Paraformer(audio_path)` → 转写为无标点文本
3. `CT_Transformer(raw_text)` → 添加标点
...
```
funasr_onnx>=0.4      # Paraformer + VAD + Punc ONNX 推理
modelscope>=1.30      # 模型下载
click>=8.1            # CLI
tqdm>=4.66            # 进度条
```

`onnxruntime` 由 `funasr_onnx` 自动带入，无需显式声明。
